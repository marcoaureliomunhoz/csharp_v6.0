...
namespace WinFormCadEditoraJSON1
{
    public partial class Form1 : Form
    {
        ...
        private void CarregarEditoras()
        {
            ...
            DataContractJsonSerializer js = new DataContractJsonSerializer(typeof(List<Editora>));
            using (MemoryStream ms = new MemoryStream(System.Text.ASCIIEncoding.ASCII.GetBytes(editoras_json)))
            {
                editoras = js.ReadObject(ms) as List<Editora>;
            }
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                foreach (var editora in editoras)
                {
                    aux +=
                        (aux.Length > 0 ? "," : "") +
                        "{" +
                        $"\"Codigo\" : \"{editora.Codigo}\", \"Nome\" : \"{editora.Nome}\"" +
                        "}";
                }
                editoras_json = "[" + aux + "]";
            }
